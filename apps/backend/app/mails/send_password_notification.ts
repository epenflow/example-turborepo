  ...
  prepare() {
    ...
    if (this.event.type === 'request') {
      ...
      htmlContent = `
        ...
        <p>
          <a href="${this.event.link}" style="background-color: #007bff; color: white; padding: 10px 20px; text-decoration: none; border-radius: 5px;">
            Reset My Password
          </a>
        </p>
        <p>This link is only valid ${this.event.expiresAt}. If you do not reset your password within this time, you will need to submit another request.</p>
    ...
    this.message.to(this.event.user.email).subject(subject).html(htmlContent)
  }
}
